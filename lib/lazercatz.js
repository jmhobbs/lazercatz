...
function LazerCatz(options) {
    ...
    self.settings = {
        ...
        userTTL: options.userTTL || 60 // seconds
    }
    ...
    var server = http.createServer(function(request, response) {
        ...
        request.addListener('end', function() {
            ...
            if (location.pathname == '/init.json' && request.method == "GET") {
                ...
                response.end(JSON.stringify( { 'you': user, 'them': self.usersByKillCount() } ));
...
LazerCatz.prototype.createSubscriptions = function() {
    ...
    self.subscribe('/die', function(user) {
        ...
        self.publish("/leaderboard", self.usersByKillCount());
    });
...
LazerCatz.prototype.usersByKillCount = function() {
    var unordered = [];
    for (var idx in users) {
        unordered.push(users[idx]);
    }

    return unordered.sort(function(a,b) {
        var x = a.kills, y = b.kills;
